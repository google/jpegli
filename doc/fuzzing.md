...
jpegli fuzzing is integrated into [oss-fuzz](https://github.com/google/oss-fuzz)
as the project `jpegli`. oss-fuzz regularly runs the fuzzers on the `main`
...
### Generate the gcr.io/oss-fuzz/jpegli image

First you need the ossfuzz jpegli builder image. This is the base oss-fuzz
...
cd ~/oss-fuzz
sudo infra/helper.py build_image jpegli
```

This will create the `gcr.io/oss-fuzz/jpegli` docker image. You can check if it
was created verifying that it is listed in the output of the `sudo docker image
ls` command.

### Build the fuzzer targets with oss-fuzz

To build the fuzzer targets from the current jpegli source checkout, use the
`./ci.sh ossfuzz_msan` command for MSan, `./ci.sh ossfuzz_asan` command for ASan
or `./ci.sh ossfuzz_ubsan` command for UBSan. All the `JPEGLI_ASSERT` and
`JPEGLI_DASSERT` calls are enabled in all the three modes. These ci.sh helpers will
reproduce the oss-fuzz docker call to build jpegli mounting the current source
 ...
 * `JPEGLI_DASSERT` is only enabled in Debug builds, which includes all the ASan,
 ...
 * For MSan builds it is also possible to specify that certain values must be
   ...
   `JPEGLI_CHECK_IMAGE_INITIALIZED(image, rect)` macro. This helps document and
...
In jpegli we often run vectorized operations over a series of values, rounding
