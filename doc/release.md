# jpegli release process

This guide documents the release process for the jpegli project.

jpegli follows the [semantic versioning](https://semver.org/spec/v2.0.0.html)
...
The following instructions assume that you followed the recommended [jpegli git
setup](developing_in_github.md) where `origin` points to the upstream
google/jpegli project, otherwise use the name of your upstream remote repository
instead of `origin`.
...
git push git@github.com:google/libjpeglijxl.git origin/main:refs/heads/v0.5.x
```

Here we use the SSH URL explicitly since you are pushing to the `google/jpegli`
...
git push git@github.com:google/jpegli.git v0.5-snapshot
```
...
In the [edit labels](https://github.com/google/jpegli/issues/labels) page, click
...
The version number (as returned by `JpegliDecoderVersion`) in the source code in
...
[CMakeLists.txt](https://github.com/google/jpegli/blob/main/lib/CMakeLists.txt#L12).
...
that example. Click on the URL displayed, which will be something like
  ...
  `https://github.com/mygithubusername/jpegli/pull/new/merge_to_release`
 ...
 * The GitHub Actions checks pass on the release branch. Look for the green
   ...
   on the branch page, for example: https://github.com/google/libjpeglijxl/tree/v0.5.x
 ...
 * There no open fuzzer-found bugs for the release branch. The most effective
   ...
   gs://jpegli-corpus.clusterfuzz-external.appspot.com/libFuzzer/jpegli_djxl_fuzzer
...
page](https://github.com/google/jpegli/releases/new) and follow these
instructions:
...
Please provide feedback and report bugs [here](https://github.com/google/jpegli/issues).
```
...
apt update
apt install -y clang cmake git nasm pkg-config ninja-build
export CC=clang
export CXX=clang++
...
git clone --recurse-submodules --depth 1 -b v0.9.x \
  https://github.com/google/jpegli.git
