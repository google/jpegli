...
#ifndef JPEGLI_COLOR_ENCODING_H_
#define JPEGLI_COLOR_ENCODING_H_
...
typedef enum {
  ...
  /** Luminance based, the primaries in @ref JpegliColorEncoding must be
   * ignored. This value implies that num_color_channels in JpegliBasicInfo is
   * 1, any other value implies num_color_channels is 3. */
  JPEGLI_COLOR_SPACE_GRAY,
  /** XYB (opsin) color space */
  JPEGLI_COLOR_SPACE_XYB,
  /** None of the other table entries describe the color space appropriately */
  JPEGLI_COLOR_SPACE_UNKNOWN,
} JpegliColorSpace;

/** Built-in white points for color encoding. When decoding, the numerical xy
 * white point value can be read from the @ref JpegliColorEncoding white_point
 * field regardless of the enum value. When encoding, enum values except
 * ::JPEGLI_WHITE_POINT_CUSTOM override the numerical fields. Some enum values
...
typedef enum {
  ...
  JPEGLI_PRIMARIES_SRGB = 1,
  /** Primaries must be read from the @ref JpegliColorEncoding primaries_red_xy,
  ...
  JPEGLI_PRIMARIES_CUSTOM = 2,
  /** As specified in Rec. ITU-R BT.2100-1 */
  JPEGLI_PRIMARIES_2100 = 9,
  /** As specified in SMPTE RP 431-2 */
  JPEGLI_PRIMARIES_P3 = 11,
} JpegliPrimaries;
...
typedef enum {
  ...
  JPEGLI_TRANSFER_FUNCTION_709 = 1,
  /** None of the other table entries describe the transfer function. */
  JPEGLI_TRANSFER_FUNCTION_UNKNOWN = 2,
  /** The gamma exponent is 1 */
  JPEGLI_TRANSFER_FUNCTION_LINEAR = 8,
  /** As specified in IEC 61966-2-1 sRGB */
  JPEGLI_TRANSFER_FUNCTION_SRGB = 13,
  /** As specified in SMPTE ST 2084 */
  JPEGLI_TRANSFER_FUNCTION_PQ = 16,
  /** As specified in SMPTE ST 428-1 */
  JPEGLI_TRANSFER_FUNCTION_DCI = 17,
  /** As specified in Rec. ITU-R BT.2100-1 (HLG) */
  JPEGLI_TRANSFER_FUNCTION_HLG = 18,
  /** Transfer function follows power law given by the gamma value in @ref
     JpegliColorEncoding. Not a CICP value. */
  JPEGLI_TRANSFER_FUNCTION_GAMMA = 65535,
} JpegliTransferFunction;

/** Rendering intent for color encoding, as specified in ISO 15076-1:2010 */
typedef enum {
  /** vendor-specific */
  JPEGLI_RENDERING_INTENT_PERCEPTUAL = 0,
  /** media-relative */
  JPEGLI_RENDERING_INTENT_RELATIVE,
  /** vendor-specific */
  JPEGLI_RENDERING_INTENT_SATURATION,
  /** ICC-absolute */
  JPEGLI_RENDERING_INTENT_ABSOLUTE,
} JpegliRenderingIntent;
...
typedef struct {
  /** Color space of the image data.
   */
  JpegliColorSpace color_space;

  /** Built-in white point. If this value is ::JPEGLI_WHITE_POINT_CUSTOM, must
   * use the numerical white point values from white_point_xy.
   */
  JpegliWhitePoint white_point;

  /** Numerical whitepoint values in CIE xy space. */
  double white_point_xy[2];

  /** Built-in RGB primaries. If this value is JPEGLI_PRIMARIES_CUSTOM, must
   ...
   * JPEGLI_COLOR_SPACE_GRAY or JPEGLI_COLOR_SPACE_XYB.
   */
  JpegliPrimaries primaries;
  ...
  JpegliTransferFunction transfer_function;

  /** Gamma value used when transfer_function is JPEGLI_TRANSFER_FUNCTION_GAMMA
   */
  double gamma;

  /** Rendering intent defined for the color profile. */
  JpegliRenderingIntent rendering_intent;
} JpegliColorEncoding;
...
#endif /* JPEGLI_COLOR_ENCODING_H_ */
