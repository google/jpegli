...
void ApplySharpYuvDownsampleImpl(j_compress_ptr cinfo) {
  ...
  for (int iter = 0; iter < 4; ++iter) {
    ...
    // 3e. Apply Cb/Cr error correction with clamping (SIMD + scalar tail)
    size_t i_c_corr = 0;
    for (; i_c_corr + N <= low_size; i_c_corr += N) {
      ...
      StoreU(Min(Max(cb_vec, vmin), vmax), d, &best_cb[i_c_corr]);
      StoreU(Min(Max(cr_vec, vmin), vmax), d, &best_cr[i_c_corr]);
    }
    for (; i_c_corr < low_size; ++i_c_corr) {
      best_cb[i_c_corr] =
          std::max(0.0f, std::min(255.0f, best_cb[i_c_corr] + err_cb[i_c_corr]));
      best_cr[i_c_corr] =
          std::max(0.0f, std::min(255.0f, best_cr[i_c_corr] + err_cr[i_c_corr]));
    }
